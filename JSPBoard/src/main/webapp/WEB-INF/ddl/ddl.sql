...
CREATE OR REPLACE TRIGGER TRG_AFILE_COUNT
AFTER
INSERT OR DELETE ON AFILE
FOR EACH ROW
BEGIN
	IF INSERTING THEN
		UPDATE ARTICLE SET AFCOUNT = AFCOUNT + 1 WHERE AID=:NEW.AID;
	ELSIF DELETING THEN 
		UPDATE ARTICLE SET AFCOUNT = AFCOUNT - 1 WHERE AID=:OLB.AID;		
	END IF;
END;

CREATE OR REPLACE TRIGGER TRG_REPLY_COUNT
AFTER
INSERT OR DELETE ON REPLY
FOR EACH ROW
BEGIN
	IF INSERTING THEN
		UPDATE ARTICLE SET ACOUNT = ACOUNT + 1 WHERE AID=:NEW.AID;
	ELSIF DELETING THEN 
		UPDATE ARTICLE SET ACOUNT = ACOUNT - 1 WHERE AID=:OLD.AID;		
	END IF;
END;
